(function () {
  ...
  function Slider(newSlider, sliderSize, slideSize, animationDuration, autoplayInterval) {
    ...
    //         const activeEl = document.querySelector('.slides-holder__item_active');
    //         const activeIndex = Array.from(_this.slides).indexOf(activeEl);
    //         const clickedIndex = index;
    
    //         if (activeIndex !== -1) {
    //             const difference = clickedIndex - activeIndex;
    //             let step = difference > 0 ? 1 : -1; // Determine direction

    //             for (let i = 0; i < Math.abs(difference); i++) {
    //                 _this.rotate(step); // Rotate one step at a time
    //             }
    //         }
    //     });
    // });
  ...
  ///////////Init sliders///////////
  window.circularSlider1 = new Slider(document.querySelector(".circular-slider-1"), 80, 10, 3000, 6000);
